    ...
    <div class="container" v-if="goods.categories">
      <div class="bread-container">
        <el-breadcrumb separator=">">
          ...
          <!-- 
            错误原因 goods一开始是{} {}.xxx 为undefined -> undefined[1]
            1、可选择的语法 `?.` 判断是否为空 
            2、v-if 手动条件渲染 保证只有数据存在时才会渲染     
          -->
          <el-breadcrumb-item :to="{ path: `/category/${goods.categories[1].id}` }"> {{ goods.categories[1].name }}
          </el-breadcrumb-item>
          <el-breadcrumb-item :to="{ path: `/categories/sub/${goods.categories[0].id}`}">{{ goods.categories[0].name}}
          </el-breadcrumb-item>
